...
def saveMask(mask):
    mask1 = mask.cpu().detach().numpy()[0]
    # tranpose the image to BGR format
    mask1 = np.transpose(mask1, (1, 2, 0))
    # normalize the mask
    mask1 = (mask1 - np.min(mask1)) / (np.max(mask1)+0.00000001)
...
def invert(image, network, layer, epochs, cuda, target_label):
    ...
    target = torch.nn.Softmax(dim=1)(logits)
    y = np.argsort(target.cpu().detach().numpy())
    category_highest = y[0][-1]
    category = y[0][-target_label]
    print("Category with highest probability", category)
    print(class_names[category], target.cpu().detach().numpy()[0][category])
    ...
    for i in range(epochs):
        ...
        # highlight loss
        logits_highlight = -1 * torch.nn.Softmax(dim=1)(model(x_))[0][category]
        # suppressing loss
        logits_supress = torch.nn.Softmax(dim=1)(model(x_deleted))[0][category]
        if i < 11:
        ...
        # if (i+1) % 1 == 0:
            # print('Epoch %d: \tTot Loss: %f' % (i+1, tot_loss.detach().cpu().numpy()[0]))
    ...
    with torch.no_grad():
        ...
        upsampled_mask.unsqueeze_(0)
    upsampled_mask = upsample(upsampled_mask)
    mask_copy = upsampled_mask.cpu().detach().numpy()[0,0]
    ...
    invert_pil = detransform(x_[0].detach().cpu())
    # invert_pil = np.array(x_[0])*255
    # invert_pil = invert_pil.astype(np.uint8)
    # invert_pil = Image.fromarray(invert_pil[0])
...
if __name__ == '__main__':
    ...
    try:
        ...
        parser.add_argument('--image', type=str, default='adv_4.jpg')
